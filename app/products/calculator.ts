...
export type Product = Prisma.ProductGetPayload<{ select: typeof select.object }>
...
const canCountPrices = (values?: Values): boolean =>
  !!values?.dollarPrice && !!values?.dollarRate &&
  !!values?.density && !!values?.width

const countPriceRubM = (values?: Values): number =>
  Number(values?.dollarPrice) * Number(values?.dollarRate) *
...
export const prices = (values?: Values): string =>

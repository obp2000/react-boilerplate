...
import { options, commonConsts, objects, cities } from '@/mocks/handlers'
...
test.describe.parallel("Customers CRUD functionality", () => {
  test("loads and displays objects", async ({ page }) => {
    ...
    await expect(heading).toContainText('45')
    ...
    const cityText = new RegExp('Владимир')
  ...
  test("displays existing object form and updates object",
    async ({ page }) => {
      ...
      await expect(formHeader).toContainText('156')

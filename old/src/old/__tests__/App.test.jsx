...
test('loads and displays objects', async () => {
  ...
  const cityName =
    screen.getByText(new RegExp(objects.results[2].city.city.split(' ')[0]))
  ...
  const deleteButtons =
    screen.getAllByRole('button', {name: commonConsts.delete})
...
test('displays new object form and creates new object', async () => {
  ...
  const tableHeader =
    await screen.findByRole('heading', {name: options.name_plural})
  expect(tableHeader).toHaveTextContent(options.name_plural)
})
...
test('displays existing object form and updates object', async () => {
  ...
  const linksToEdit =
    await screen.findAllByRole('link', {name: commonConsts.edit})
  expect(linksToEdit).toHaveLength(8)
  expect(linksToEdit[2]).toHaveTextContent(commonConsts.edit)
  expect(linksToEdit[2]).toHaveAttribute('href',
...
test('deletes object', async () => {
  ...
  const deleteButtons =
    await screen.findAllByRole('button', {name: commonConsts.delete})
  expect(deleteButtons).toHaveLength(8)
  ...
  const cancelDeleteButton =
    getByRole(confirmation, 'button', {name: commonConsts.no})
  ...
  const proceedDeleteButton =
    getByRole(confirmation, 'button', {name: commonConsts.yes})
